...
def extract_nt_sequence(GenBank_file, feature):
    ...
    try:
        ...
        return (
            str(GenBank_file.seq[start - 1 : end]),
            start,
            end,
...
def split_gbk_by_protocluster_fna(filepath, output_dir):
    ...
    for cluster_id, record_features in clusters.items():
        ...
        functional_indices = [
            i
            for i, (_, feat) in enumerate(record_features)
        ...
        if not functional_indices:
            ...
            print(
                f"Skipping protocluster {cluster_id} - no functional CDS features found"
            )
            continue
        ...
        for region_name, features in regions.items():
            ...
            out_file = os.path.join(
                output_dir, f"{basename}_protocluster_{cluster_id}_{region_name}.fna"
            )
            ...
            for record, feat in features:
                ...
                if nt_seq:
                    ...
                    seq_records.append(
                        SeqRecord(Seq(nt_seq), id=header, description=f"")
                    )
